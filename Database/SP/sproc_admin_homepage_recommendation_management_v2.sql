...
/****** Object:  StoredProcedure [dbo].[sproc_admin_homepage_recommendation_management_v2]    Script Date: 05/12/2023 15:35:12 ******/
SET ANSI_NULLS ON
GO
...
ALTER PROC [dbo].[sproc_admin_homepage_recommendation_management_v2]
    @Flag VARCHAR(10),
...
DECLARE @MaxNoOfClub INT = 10,
        @DisplayPageId VARCHAR(10) = 1;
BEGIN TRY
    IF @Flag = 'cchpr' --create club home page recommendation
    BEGIN
        IF EXISTS
        (
            SELECT 'X'
        ...
        BEGIN
            SELECT 1 Code,
                   'Home page recommendation already exists for the given club' Message;
            RETURN;
        END;
        ...
        BEGIN
            SELECT 1 Code,
                   'Home page recommendation already consist of maximum number of club' Message;
            RETURN;
        END;
        ...
        SET @TransactionName = 'Transaction_cchpr';
        ...
        SELECT 0 Code,
               'Recommendation for home page added successfully for the club' Message;
        RETURN;
    END;
    ...
    ELSE IF @Flag = 'dhpr' --delete home page recommendation
    BEGIN
        IF NOT EXISTS
        (
            SELECT 'X'
        ...
        SELECT 0 Code,
               'Home page recommendation removed succussfully for the given club' Message;
        RETURN;
    END;
END TRY
BEGIN CATCH
    ...
    VALUES
    (@ErrorDesc, 'sproc_admin_homepage_recommendation_management_v2(Flag: ' + ISNULL(@Flag, '') + ')', 'SQL', 'SQL',
     'sproc_admin_homepage_recommendation_management_v2', GETDATE());
