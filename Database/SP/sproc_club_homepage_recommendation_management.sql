ALTER PROC [dbo].[sproc_club_homepage_recommendation_management]
    @Flag VARCHAR(10),
...
DECLARE @DisplayPageId VARCHAR(10) = 1;
BEGIN TRY
    IF @Flag = 'ghpac' --get home page assigned club
    BEGIN
        SELECT DISTINCT
    ...
    ELSE IF @Flag = 'cchpr' --create club home page recommendation
    BEGIN
        IF NOT EXISTS
        (
            SELECT 'X'
...
BEGIN CATCH
    ...
    VALUES
    (@ErrorDesc, 'sproc_club_homepage_recommendation_management(Flag: ' + ISNULL(@Flag, '') + ')', 'SQL', 'SQL',
     'sproc_club_homepage_recommendation_management', GETDATE());
