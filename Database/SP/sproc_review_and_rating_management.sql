...
/****** Object:  StoredProcedure [dbo].[sproc_review_and_rating_management]    Script Date: 27/11/2023 14:29:44 ******/
SET ANSI_NULLS ON;
GO
...
ALTER PROC [dbo].[sproc_review_and_rating_management]
    @Flag VARCHAR(10),
...
BEGIN TRY
    ...
    BEGIN
        ...
        (
            SELECT 'X'
            FROM dbo.tbl_reservation_detail a WITH (NOLOCK)
                INNER JOIN dbo.tbl_review_and_rating b WITH (NOLOCK)
                    ON b.ReservationId = a.ReservationId
        ...
        SELECT 0 Code,
               ...
               CONCAT(B.PlanName, ' (¥', ISNULL(B.Price, 0), ')') AS Price,
               ISNULL(A.NoOfPeople, 0) AS NoOfPeople,
               0 AS UsedPoint,
               CONCAT('¥', ISNULL((B.Price * A.NoOfPeople), 0)) AS TotalAmount,
               E.StaticDataLabel AS PaymentMethod,
               d.AgentId AS ClubId,
               d.Logo AS ClubLogo,
    ...
    BEGIN
        ...
        (
            ...
            FROM dbo.tbl_customer A WITH (NOLOCK)
                INNER JOIN dbo.tbl_reservation_detail B WITH (NOLOCK)
                    ON B.CustomerId = A.AgentId
                       AND ISNULL(B.TransactionStatus, '') IN ( 'A' )
            ...
            FROM dbo.tbl_club_details a WITH (NOLOCK)
                INNER JOIN dbo.tbl_host_details b WITH (NOLOCK)
                    ON b.AgentId = a.AgentId
                       AND b.HostId IN
                           (
                               SELECT * FROM STRING_SPLIT(ISNULL(@HostList, ''), ',')
                           )
        )
        BEGIN
            SELECT 1 Code,
        ...
        CREATE TABLE #temp_mr_1
        (
            Value VARCHAR(MAX)
        );

        INSERT INTO #temp_mr_1
        (
            Value
        )
        SELECT *
        ...
        (   @ReservationId,   -- ReservationId - bigint
            ...
            @HostList,        -- HostList - varchar(max)
            @MVPHostId,       --- MVPHostId
            @RemarkList,      -- RemarkList - varchar(max)
            @RatingScale,     -- RatingScale - int
            'A', @ActionUser, -- ActionUser - varchar(200)
            GETDATE(),        -- ActionDate - bigint
		...
		';
        SET @StringSQL += N'SELECT ' + CAST(@Sno AS VARCHAR) + N', a.QuestionId, a.AnswerId,'''
                          + CAST(@ActionUser AS VARCHAR) + N''', GETDATE(),''' + @ActionIP + N''',''' + @ActionPlatform
                          + N'''
...
BEGIN CATCH
    ...
    VALUES
    (@ErrorDesc, 'sproc_review_and_rating_management(Flag: ' + ISNULL(@Flag, '') + ')', 'SQL', 'SQL',
     'sproc_review_and_rating_management', GETDATE());
